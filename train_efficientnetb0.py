"""
EfficientNet-B0 Training Script - 3-Class Classification
"""
...
def create_working_model():
    """Create a fresh EfficientNetB0 model that will work"""
    
    print("Creating fresh EfficientNetB0 model...")
...
def train_working_model():
    ...
    print("SkinVestigator AI - Training EfficientNet-B0 (3-Class)")
    print("=" * 60)
    ...
    # Get the base model - find the EfficientNetB0 layer
    base_model = None
    for layer in model.layers:
        if 'efficientnet' in str(type(layer).__name__).lower():
            base_model = layer
            break
    
    if base_model is None:
        print("Warning: Could not find EfficientNet base model. Creating training info anyway.")
        base_model = model  # Fallback to full model
    
    print(f"Found base model: {type(base_model).__name__}")
    
    # Create training info even if Phase 1 only
    training_info = {
        'model_path': model_path,
        'training_date': datetime.now().isoformat(),
        'validation_accuracy': max(history_phase1.history['val_accuracy']),
        'validation_auc': max(history_phase1.history['val_auc']),
    ...
    with open("models/efficientnetb0_training_info.json", 'w') as f:
    ...
    # Save model
    model_path = "models/efficientnetb0_new.keras"
...
if __name__ == "__main__":
    ...
    if model:
        print(f"\n" + "="*60)
        print("SUCCESS: EfficientNet-B0 model trained and saved!")
