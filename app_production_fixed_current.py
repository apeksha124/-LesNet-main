...
class GradCAMGenerator:
    ...
    def overlay_heatmap(self, original_img, heatmap, alpha=0.4):
        """Overlay heatmap on original image - Fixed to create actual heatmap"""
        try:
            print(f"Original image shape: {original_img.shape}")
            print(f"Heatmap shape: {heatmap.shape}")
            print(f"Heatmap min/max: {heatmap.min():.6f}/{heatmap.max():.6f}")
            
            # Ensure original_img is uint8 and proper range
            ...
            # Create a proper heatmap if the input heatmap is invalid
            if heatmap is None or np.max(heatmap) <= 1e-6:
                print("Creating synthetic heatmap - original heatmap invalid")
                h, w = original_img.shape[:2]
                # Create a centered circular heatmap
                center_x, center_y = w // 2, h // 2
                y, x = np.ogrid[:h, :w]
                mask = (x - center_x) ** 2 + (y - center_y) ** 2
                heatmap = np.exp(-mask / (2 * (min(w, h) // 6) ** 2))
                print(f"Created synthetic heatmap with max: {heatmap.max():.3f}")
...
class MultiModelEnsemble:
    ...
    def _load_model_ensemble(self):
        ...
        try:
            ...
            model_configs = {
                'normal_skin_3class': {
                    'path': 'models/normal_skin_3class.keras',  # 3-class model (TensorFlow compatible)
                    'weight': 0.25,
                    'name': 'Normal Skin 3-Class Model',
                    'description': '3-class model (NORMAL SKIN, benign, malignant)'
                },
                'efficientnet_b0': {
                    'path': 'models/efficientnetb0_new.keras',  # 3-class model
                    'weight': 0.25,
                    'name': 'EfficientNet-B0 (3-Class)',
                    'description': '3-class model (NORMAL SKIN, benign, malignant)'
                },
                'efficientnet_b3': {
                    'path': 'models/efficientnetb3_new.keras',  # 3-class model
                    'weight': 0.25,
                    'name': 'EfficientNet-B3 (3-Class)',
                    'description': '3-class model (NORMAL SKIN, benign, malignant)'
                },
                'resnet50': {
                    'path': 'models/resnet50_new.keras',  # 3-class model
                    'weight': 0.25,
                    'name': 'ResNet-50 (3-Class)',
                    'description': '3-class model (NORMAL SKIN, benign, malignant)'
                }
            }
            ...
            for model_id, config in model_configs.items():
                try:
                    print(f"DEBUG: Checking model {model_id} at path {config['path']}")
                    if os.path.exists(config['path']):
                        print(f"DEBUG: Loading model from {config['path']}")
                        try:
                            ...
                            print(f"DEBUG: Successfully loaded {model_id}")
    ...
    def _make_ensemble_prediction(self, image_array):
        ...
        try:
            print("Running ensemble prediction with {} models".format(len(self.models)))

            # Collect predictions from all loaded models (models already loaded)
            predictions = []
            confidences = []
            model_results = []
            class_indices = []
            ...
            for model_id, model in self.models.items():
                if model is not None:
                    try:
                        # Use model-specific preprocessing - call from parent app
                        # Get reference to main app for preprocessing
                        main_app = st.session_state.get('skin_app')
                        if main_app and hasattr(main_app, 'preprocess_image_for_model'):
                            model_input = main_app.preprocess_image_for_model(original_image, model_id)
                        else:
                            # Fallback to basic preprocessing
                            model_input = efficientnet_preprocess(original_image.astype(np.float32))
                        # Add batch dimension
                        ...
                        # DEBUG: Show full prediction array
                        print(f"  DEBUG {model_id}: prediction={prediction[0]} (shape={prediction[0].shape})")
                        print(f"  DEBUG {model_id}: pred_class_idx={pred_class_idx}, confidence={confidence:.3f}")
                        
                        predictions.append(prediction[0])
                        confidences.append(confidence)
                        class_indices.append(pred_class_idx)
            ...
            # Handle 50% ties with confidence-based tie-breaking
            if agreement_rate == 0.5:
                print(f"50% tie detected - using confidence-based tie-breaking")
                # Group predictions by class with their confidences
                class_confidences = {}
                ...
                print(f"Class confidences: {avg_confidences}")
                
                # Choose class with highest average confidence
                ensemble_class = max(avg_confidences, key=avg_confidences.get)
                ensemble_confidence = avg_confidences[ensemble_class]
                print(f"Tie broken: class {ensemble_class} with confidence {ensemble_confidence:.3f}")
            
            # Calculate confidence variance
            confidences = [r['confidence'] for r in model_results]
            confidence_variance = np.var(confidences)
            
            # Apply medical confidence calibration (prevent overconfidence)
            # Reasoning: Medical diagnosis should never claim 100% certainty
            # High ensemble agreement + mathematical artifacts can create unrealistic confidence
            if ensemble_confidence > 0.95:
                # Apply temperature scaling for extreme confidence values
                # This preserves relative confidence while preventing unrealistic certainty
                temperature = 2.0  # Softens extreme probabilities
                calibrated_pred = np.power(ensemble_pred, 1/temperature)
                calibrated_pred = calibrated_pred / np.sum(calibrated_pred)
                ensemble_confidence = float(np.max(calibrated_pred))
                
                # Additional safety: ensure minimum uncertainty for medical decisions
                if ensemble_confidence > 0.98:
                    ensemble_confidence = 0.98  # Maximum 98% for medical safety
                
                print(f"Applied confidence calibration: {ensemble_confidence:.3f} (was {np.max(ensemble_pred):.3f})")
...
class ProductionSkinVestigatorApp:
    ...
    def _load_production_model(self):
        ...
        try:
            ...
            if self.model_ensemble.ensemble_loaded:
                ...
                # Use first available model for Grad-CAM
                if self.model_ensemble.ensemble_loaded and len(self.model_ensemble.models) > 0:
                    first_model_id = list(self.model_ensemble.models.keys())[0]
                    self.model = self.model_ensemble.models[first_model_id]
    ...
    def preprocess_image_from_file(self, image_file):
        ...
        try:
            ...
            processed = self.preprocess_image_for_model(image, 'efficientnet_b0')
    ...
    def preprocess_image_for_model(self, image, model_id):
        ...
        try:
            ...
            if model_id == 'efficientnet_b3':
                # EfficientNet-B3 uses 300x300
                image_resized = cv2.resize(image, (300, 300))
                # Use EfficientNet preprocessing
                processed = efficientnet_preprocess(image_resized.astype(np.float32))
            elif model_id == 'resnet50':
                # ResNet-50 uses 224x224
                image_resized = cv2.resize(image, (224, 224))
                # Use ResNet preprocessing
                processed = resnet_preprocess(image_resized.astype(np.float32))
            elif model_id == 'vision_transformer':
                # Vision Transformer uses 224x224 with normalization
                image_resized = cv2.resize(image, (224, 224))
                # Normalize to [0, 1] for ViT
                processed = image_resized.astype(np.float32) / 255.0
            else:
                # Default: EfficientNet-B0 uses 224x224
                image_resized = cv2.resize(image, (224, 224))
                # Use EfficientNet preprocessing
                processed = efficientnet_preprocess(image_resized.astype(np.float32))
            ...
            print(f"Model preprocessing error for {model_id}: {str(e)}")
            # Fallback to basic preprocessing
            image_resized = cv2.resize(image, (224, 224))
            processed = efficientnet_preprocess(image_resized.astype(np.float32))
            return processed

    def preprocess_image_from_upload(self, uploaded_file):
        """Enhanced image preprocessing for real-world medical images"""
        try:
            # Validate input
            ...
            # Apply default preprocessing (EfficientNet-B0)
            processed = self.preprocess_image_for_model(display_image, 'efficientnet_b0')
    ...
    def preprocess_image_from_camera(self, camera_image):
        """Camera image preprocessing - using same logic as upload"""
        try:
            ...
            # Use PIL to handle camera image (same as upload fallback)
            from PIL import Image
            pil_image = camera_image if hasattr(camera_image, 'convert') else Image.open(camera_image)
            
            # Convert PIL to numpy array (already in RGB)
            image = np.array(pil_image)
            print(f"PIL Camera image shape: {image.shape}, dtype: {image.dtype}")
            
            # Ensure it's in RGB format (same as upload logic)
            if len(image.shape) == 3 and image.shape[2] == 4:
                # Remove alpha channel
                image = image[:, :, :3]
            elif len(image.shape) == 2:
            ...
            print(f"Final camera image shape: {image.shape}, dtype: {image.dtype}")
            
            # Check if demo mode is active
            if st.session_state.get('demo_mode', False):
                print("DEMO MODE: Using original camera image")
                enhanced_image = image
            else:
                # Use original camera image without enhancement (same as upload)
                enhanced_image = image
            
            # Process image for model input (use EfficientNet preprocessing as default)
            processed_image = cv2.resize(enhanced_image, (224, 224))
            processed = efficientnet_preprocess(processed_image.astype(np.float32))
            processed = np.expand_dims(processed, axis=0)
            
            print("Camera image processing completed successfully")
            
            # Return processed image and original display image
            return processed, image
            
        except Exception as e:
            print(f"Camera image processing error: {str(e)}")
            import traceback
            traceback.print_exc()
            return None, None
    ...
    def _detect_clear_skin(self, image):
        """Detect if image shows clear skin without lesions"""
        try:
            ...
            mean_r = np.mean(img[:, :, 0])  # Red channel
            mean_g = np.mean(img[:, :, 1])  # Green channel
            mean_b = np.mean(img[:, :, 2])  # Blue channel
            
            # Edge detection to find lesions
            edges = cv2.Canny(gray, 30, 100)
            edge_density = np.sum(edges > 0) / (edges.shape[0] * edges.shape[1])
            
            # Color uniformity (clear skin has uniform color)
            color_std = np.std(gray)
            
            # Clear skin indicators:
            # 1. Low variance (uniform texture)
            # 2. Low edge density (no lesion boundaries)
            # 3. High color uniformity
            # 4. Skin-like color ratios
            is_clear_skin = (
                variance < 8000 and  # Low texture variance
                edge_density < 0.02 and  # Few edges
                color_std < 40 and  # Uniform color
                mean_r > 80 and mean_r < 200 and  # Skin-like red
                mean_g > 60 and mean_g < 180 and  # Skin-like green
                mean_b > 50 and mean_b < 150  # Skin-like blue
            )
            
            print(f"Clear skin analysis: variance={variance:.0f}, edge_density={edge_density:.3f}, color_std={color_std:.1f}")
            print(f"Color means: R={mean_r:.1f}, G={mean_g:.1f}, B={mean_b:.1f}")
            print(f"Clear skin detected: {is_clear_skin}")
            
            return is_clear_skin
            
        except Exception as e:
            print(f"Clear skin detection failed: {e}")
            return False
    ...
    def _make_ensemble_prediction(self, image_array):
        ...
        try:
            ...
            ensemble_pred, ensemble_confidence, ensemble_details = self.model_ensemble._make_ensemble_prediction(image_array)
            
            if ensemble_pred is None:
                print("Ensemble prediction failed, falling back to single model")
                return self._make_single_prediction(image_array)
            
            # Add original image to ensemble details for clear skin detection
            # Convert image_array back to original format for analysis
            original_image = np.clip(image_array[0] + [123.675, 116.28, 103.53], 0, 255).astype(np.uint8)
            ensemble_details['original_image'] = original_image
            
            # Extract results for 3 classes (newly trained)
            ...
            if st.session_state.get('demo_mode', False):
                ...
                # 2-class models don't have "no lesion" - they only predict benign/malignant
                # No clear skin detection for 2-class models
            else:
                # Handle 3 classes properly with RAW CONFIDENCE (no artificial boosting)
                # Get model agreement details for display only (no confidence modification)
                agreement_rate = ensemble_details.get('agreement_rate', 0.0)
                individual_predictions = ensemble_details.get('individual_predictions', {})
                
                # Handle 3-class models correctly
                if individual_predictions:
                    # Get the first prediction to check shape
                    first_pred = list(individual_predictions.values())[0]
                    # Check if it's a numpy array with 3 classes (not a scalar)
                    if hasattr(first_pred, 'shape') and len(first_pred.shape) > 0 and first_pred.shape[-1] == 3:
                        # 3-class model: 0=NORMAL SKIN, 1=benign, 2=malignant
                        if pred_class_idx == 0:
                            print("3-class model: NORMAL SKIN detected")
                            pred_class = 'No lesion detected'
                        elif pred_class_idx == 1:
                            print("3-class model: Benign detected")
                            pred_class = 'benign'
                        elif pred_class_idx == 2:
                            print("3-class model: Malignant detected")
                            pred_class = 'malignant'
                        else:
                            pred_class = 'benign'  # Fallback
                            print(f"DEBUG ENSEMBLE: Unknown class index {pred_class_idx}, falling back to benign")
                    else:
                        # For safety, handle any unexpected model types
                        print(f"DEBUG ENSEMBLE: Unexpected model type, using fallback logic")
                        if pred_class_idx == 0:
                            pred_class = 'No lesion detected'
                        elif pred_class_idx == 1:
                            pred_class = 'benign'
                    ...
                    # No individual predictions available, assume 2-class
                    print("DEBUG ENSEMBLE: No individual predictions, assuming 2-class models")
                    if pred_class_idx == 0:
                        pred_class = 'benign'
                    elif pred_class_idx == 1:
                        pred_class = 'malignant'
    ...
    def _make_single_prediction(self, image_array):
        ...
        try:
            ...
            if st.session_state.get('demo_mode', False):
                ...
                # Handle both 2-class and 3-class model outputs
                print(f"DEBUG SINGLE: pred_class_idx = {pred_class_idx}, confidence = {confidence:.3f}")
                print(f"DEBUG SINGLE: raw prediction = {prediction}")
                print(f"DEBUG SINGLE: prediction shape = {prediction.shape}")
                
                # Check if model outputs 2 classes or 3 classes
                if len(prediction) == 2:
                    # 2-class model: 0=benign, 1=malignant
                    print("DEBUG SINGLE: 2-class model detected")
                    if pred_class_idx == 0:
                        pred_class = 'benign'
                    elif pred_class_idx == 1:
                        pred_class = 'malignant'
                    ...
                    # 3-class model: NORMAL SKIN=0, benign=1, malignant=2
                    print("DEBUG SINGLE: 3-class model detected")
                    if pred_class_idx == 0:
                        # Class 0 is NORMAL SKIN
                        print("Normal skin detected - no lesion no cancer")
                        pred_class = 'no lesion no cancer'
                    elif pred_class_idx == 1:
                        # Class 1 is benign
                        pred_class = 'benign'
                    elif pred_class_idx == 2:
                        # Class 2 is malignant
                        pred_class = 'malignant'
                    else:
                        pred_class = 'benign'  # Fallback
                        print(f"DEBUG SINGLE: Unknown class index {pred_class_idx}, falling back to benign")
    ...
    def generate_gradcam_visualization(self, image_array, original_image, pred_class_idx):
        """Generate Grad-CAM visualization - Working Version"""
        ...
        try:
            ...
            print("🔄 Creating fallback Grad-CAM visualization")
            
            # Ensure original image is proper format
            ...
            print(f"Gray image min/max: {gray.min()}/{gray.max()}")
            ...
            # If no significant features found, create meaningful heatmap
            if np.max(heatmap) < 0.1:
                print("No significant features detected, creating centered heatmap")
                # Create a centered circular heatmap to show something meaningful
                center_x, center_y = w // 2, h // 2
                y, x = np.ogrid[:h, :w]
                mask = (x - center_x) ** 2 + (y - center_y) ** 2
                heatmap = np.exp(-mask / (2 * (min(w, h) // 4) ** 2))
                print(f"Created centered heatmap with max: {heatmap.max():.3f}")
            ...
            print(f"Final heatmap min/max: {heatmap.min():.3f}/{heatmap.max():.3f}")
            ...
            print("✅ Fallback Grad-CAM generated successfully")
            return gradcam_img, heatmap
    ...
    def render_analysis_page(self):
        ...
        # Simple Image Input - Both Upload and Camera Options
        st.markdown('<div class="result-box">', unsafe_allow_html=True)
        
        # Tab layout for upload and camera
            ...
            if uploaded_file:
                try:
                    ...
                    if image_array is not None:
                        ...
                        # Store for analysis - ensure proper image format
                        st.session_state.current_image_array = image_array
                        # Ensure original image is properly formatted for display
                        if original_image.dtype != np.uint8:
                            if original_image.max() <= 1.0:
                                st.session_state.current_original_image = (original_image * 255).astype(np.uint8)
                            else:
                                st.session_state.current_original_image = np.clip(original_image, 0, 255).astype(np.uint8)
                        else:
                            st.session_state.current_original_image = original_image.copy()
                        st.session_state.image_source = 'upload'
                        print(f"Stored upload image - shape: {st.session_state.current_original_image.shape}, dtype: {st.session_state.current_original_image.dtype}")
        ...
        with tab2:
            ...
            camera_image = st.camera_input("Take a photo of the skin lesion")
            
            if camera_image:
                try:
                    # Clear previous results when new camera image is captured
                    st.session_state.current_prediction = None
                    
                    print("Processing camera image...")
                    image_array, original_image = self.preprocess_image_from_camera(camera_image)
                    
                    print(f"Camera processing result: image_array={image_array is not None}, original_image={original_image is not None}")
                    
                    if image_array is not None and original_image is not None:
                        st.image(original_image, caption="Camera Image", width=300)
                        ...
                        print(f"Stored camera image - shape: {st.session_state.current_original_image.shape}, dtype: {st.session_state.current_original_image.dtype}")
                        
                        # Analyze button
                        if st.button("Analyze Camera Image", type="primary", use_container_width=True):
                            self.perform_simple_analysis()
                    else:
                        st.error("Camera image processing failed - no image data")
                        print("Camera processing failed - image_array or original_image is None")
                            
                except Exception as e:
                    st.error(f"Error processing camera image: {str(e)}")
                    print(f"Camera processing exception: {str(e)}")
                    import traceback
                    traceback.print_exc()
        
        # Common analyze button if image is loaded from either source
        if 'current_image_array' in st.session_state and st.session_state.current_image_array is not None:
            st.markdown("---")
            if st.button("🔍 Analyze Image", type="primary", use_container_width=True):
                self.perform_simple_analysis()
        ...
        try:
            ...
            if prediction is not None or st.session_state.get('demo_mode', False):
                ...
                with col2:
                    ...
                    confidence_display = f"{confidence:.1%}" if confidence > 0 else "Low"
                    st.markdown(f"### Prediction: **{pred_class}** (Confidence: {confidence_display})")
                    
                    # Show model agreement if ensemble was used
                    if metadata and 'ensemble_used' in metadata and metadata['ensemble_used']:
                        st.markdown(f"### Model Agreement: {metadata.get('agreement_rate', 0):.1%}")
                        st.markdown(f"### Models Used: {metadata.get('model_count', 0)}/5")
                    
                    # Show individual model results if available
                    if metadata and 'model_results' in metadata and metadata['model_results']:
                        ...
                        for result in metadata['model_results']:
                            ...
                            # Convert class index to readable format
                            # Check if model is 3-class or 2-class based on model name
                            if "3-Class" in model_name or "Normal Skin" in model_name:
                                # 3-class model: 0=NORMAL SKIN, 1=benign, 2=malignant
                                if model_pred == 0:
                                    pred_text = "No lesion detected"
                                elif model_pred == 1:
                                    pred_text = "Benign"
                                elif model_pred == 2:
                                    pred_text = "Malignant"
                                else:
                                    pred_text = f"Class {model_pred}"
                            else:
                                # 2-class model: 0=benign, 1=malignant
                                if model_pred == 0:
                                    pred_text = "Benign"
                                    ...
                                    pred_text = f"Class {model_pred}"
                            st.markdown(f"- **{model_name}**: {pred_text} ({model_conf:.1%})")
        ...
        if ensemble_meta.get('ensemble_used', False):
            ...
            if ensemble_meta.get('model_results'):
                ...
                for result in ensemble_meta['model_results']:
                    # Convert class index to readable format for 3-class
                    pred_class = result['pred_class']
                    if pred_class == 0:
                        pred_text = "No lesion detected"
                    elif pred_class == 1:
...
if __name__ == "__main__":
    ...
    # Store app in session state for ensemble access
    st.session_state['skin_app'] = app
    app.run()
